...
//tbr import { useSessionValue } from '../../context';
import { reset } from '../../../storage/store';

const getPageTitle = page => {
  switch (page) {
    case 0:
      return 'Based on today’s session';
    case 1:
      return 'Make a plan';
    case 2:
      return 'Prepare for change';
    default:
      return '';
  }
};
...
const getPageButtons = (page, setPage, makeReflectionDoc) => {
  ...
  switch (page) {
    case 0:
      return (
        <ColorLibNextButton variant="contained" size="medium" onClick={handleNext}>
          Next
        </ColorLibNextButton>
      );
    case 1:
      return (
        <div>
          <ColorLibBackButton style={{ margin: '0px 8px' }} variant="outlined" size="medium" onClick={handleBack}>
            Back
          </ColorLibBackButton>
          <ColorLibNextButton style={{ margin: '0px 8px' }} variant="contained" size="medium" onClick={handleNext}>
            Next
          </ColorLibNextButton>
        </div>
      );
